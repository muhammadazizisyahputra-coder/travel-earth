...
（如未收到，请检查垃圾邮件箱）`;const l=setTimeout(()=>{o.style.color="",ce("login"),delete e.dataset._registerTimer},3500);e.dataset._registerTimer=String(l)}}catch(a){const i=qe(a.message);o.style.color="",o.textContent=i,console.error("[auth] 注册失败:",a.message,"| 原始错误:",a)}finally{s.disabled=!1,s.textContent="注册"}}),e.addEventListener("keydown",t=>{t.key==="Enter"&&(e.querySelector("#auth-form-login").style.display!=="none"?e.querySelector("#auth-login-submit").click():e.querySelector("#auth-register-submit").click())})}function un(){const e=document.createElement("div");e.id="edit-profile-modal",e.className="auth-modal",e.innerHTML=`
    <div class="auth-modal-overlay"></div>
    <div class="auth-modal-panel">
      <button class="auth-modal-close">&times;</button>
      <div class="auth-form">
  ...
  `,e.querySelector(".spot-list-close").addEventListener("click",Te),e.addEventListener("click",t=>{t.target===e&&Te()}),e}function yt(){k&&(T=k.querySelector("#spot-list-toggle"),T&&T.addEventListener("click",e=>{e.stopPropagation(),re?Te():Cn()}))}function Cn(){k&&(k.classList.add("open"),re=!0,He())}function He(){if(!T)return;const e=T.querySelector(".spot-list-toggle-arrow");re?(e.style.transform="rotate(180deg)",T.title="收起景区列表",T.classList.add("spot-list-toggle--open")):(e.style.transform="rotate(0deg)",T.title="展开景区列表",T.classList.remove("spot-list-toggle--open"))}function _n(e,t){k||(k=ht(),document.body.appendChild(k),yt());const n=document.getElementById("spot-list-title"),r=document.getElementById("spot-list-count"),o=document.getElementById("spot-list-body");n&&(n.textContent=t||"📍 景区列表"),r&&(r.textContent=e?`${e.length} 个`:""),!e||e.length===0?o&&(o.innerHTML='<p class="spot-list-empty">暂无景区数据</p>'):o&&(o.innerHTML=e.map(s=>`
      ...
      `).join(""),o.querySelectorAll(".spot-list-item").forEach(s=>{s.addEventListener("click",()=>{const a=Number(s.dataset.spotId),i=parseFloat(s.dataset.lng),l=parseFloat(s.dataset.lat),u=s.dataset.name,v=s.dataset.desc,x=s.dataset.isHot==="1";ne&&ne(a,i,l,u,v,x)})})),k.classList.add("open"),re=!0,He()}function Te(){k&&k.classList.remove("open"),re=!1,He()}function gt(){const e=document.createElement("div");return e.id="hot-ranking-panel",e.className="hot-ranking-panel",e.innerHTML=`
  ...
  `,e.querySelector(".hot-ranking-close").addEventListener("click",vt),e}async function kn(e){const t=document.getElementById("hot-ranking-list");if(t)try{const n=await e(10);if(!n||n.length===0){t.innerHTML='<p class="hot-ranking-empty">暂无热门景区数据</p>';return}t.innerHTML=n.map((r,o)=>`
