(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const r of document.querySelectorAll('link[rel="modulepreload"]'))n(r);new MutationObserver(r=>{for(const a of r)if(a.type==="childList")for(const s of a.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&n(s)}).observe(document,{childList:!0,subtree:!0});function o(r){const a={};return r.integrity&&(a.integrity=r.integrity),r.referrerPolicy&&(a.referrerPolicy=r.referrerPolicy),r.crossOrigin==="use-credentials"?a.credentials="include":r.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function n(r){if(r.ep)return;r.ep=!0;const a=o(r);fetch(r.href,a)}})();const Le="7dfc44451d8128e329100a0c71fa90b6",ke="db200c6e5adf1ae0023dc0d1f8a4e906";window._AMapSecurityConfig={securityJsCode:ke};window.__amapPromise=new Promise((e,t)=>{const o=document.createElement("script");o.src=`https://webapi.amap.com/maps?v=2.0&key=${encodeURIComponent(Le)}`,o.onload=()=>{console.log("[index] 高德地图 SDK 加载完成"),e()},o.onerror=()=>{console.error("[index] 高德地图 SDK 加载失败"),t(new Error("高德地图 SDK 加载失败"))},document.head.appendChild(o)});let g=null,c=null,h=null,k=!0,te=new Set,ee=null;const m={get isLoggedIn(){return!!c},get loading(){return k},get user(){return c},get profile(){return h},get supabase(){return g},async init(e){if(g)return console.warn("[authStore] 已初始化，跳过重复调用"),ee;g=e;const o=setTimeout(()=>{k&&(console.warn("[authStore] ⚠️ 安全网触发：Supabase %s 超时，强制 _loading = false",g!=null&&g.auth?"getSession":"未初始化"),k=!1,$())},3e3),{data:n}=g.auth.onAuthStateChange(Ie);return n==null||n.subscription,ee=g.auth.getSession().then(async({data:{session:r}})=>{clearTimeout(o);const a=(r==null?void 0:r.user)??null;c&&a&&c.id===a.id||(c=a,c?await ve():h=null),k=!1,$()}).catch(r=>{clearTimeout(o),console.error("[authStore] getSession 失败:",r),k=!1,$()}),ee},async signIn(e,t){if(!g)throw new Error("Supabase 客户端未初始化");const{data:o,error:n}=await B(g.auth.signInWithPassword({email:e,password:t}),15e3,"登录请求超时，请检查网络后重试");if(n)throw n;return o},async signUp(e,t,o){if(!g)throw new Error("Supabase 客户端未初始化");const{data:n,error:r}=await B(g.auth.signUp({email:e,password:t,options:{data:{display_name:o}}}),15e3,"注册请求超时，请检查网络后重试");if(r)throw r;return n},async signOut(){if(g){try{await B(g.auth.signOut(),1e4,"注销请求超时")}catch(e){console.error("[authStore] signOut 失败:",e)}c=null,h=null,$()}},async updateProfile(e){if(!c)throw new Error("未登录");const{data:t,error:o}=await g.from("profiles").update(e).eq("id",c.id).select().single();if(o)throw o;return h=t,$(),t},subscribe(e){te.add(e);try{e(we())}catch(t){console.warn("[authStore] subscribe 初始回调出错:",t)}return()=>{te.delete(e)}},getAvatarUrl(){return h!=null&&h.avatar_url?h.avatar_url:`https://api.dicebear.com/7.x/avataaars/svg?seed=${(c==null?void 0:c.id)||"default"}`},getDisplayName(){var e,t;return(h==null?void 0:h.display_name)||((e=c==null?void 0:c.user_metadata)==null?void 0:e.display_name)||((t=c==null?void 0:c.email)==null?void 0:t.split("@")[0])||"用户"}};async function Ie(e,t){var o;console.log("[authStore] 认证事件:",e,(o=t==null?void 0:t.user)==null?void 0:o.email);try{const n=(t==null?void 0:t.user)??null,r=c&&n&&c.id!==n.id||!c&&n||c&&!n;c=n,c&&r?await ve():c||(h=null)}catch(n){console.error("[authStore] onAuthStateChange 处理异常:",n),h=c?O():null}k&&(k=!1),$()}async function ve(){var o,n,r,a,s;if(!c)return;let e,t;try{const i=await B(g.from("profiles").select("*").eq("id",c.id).maybeSingle(),8e3,"profiles 查询超时");e=i.data,t=i.error}catch(i){(o=i.message)!=null&&o.includes("超时")?console.warn("[authStore] profiles 查询超时，使用兜底 profile"):console.error("[authStore] profiles 查询网络异常:",i.message),h=O();return}if(t){const i=t==null?void 0:t.code,l=(t==null?void 0:t.hint)||"";i==="PGRST301"||l.includes("JWT")?console.warn("[authStore] profiles 查询 401 (JWT):",t.message):l.includes("permission")||i==="42501"?console.error("[authStore] profiles 查询 403 (RLS):",t.message):console.warn("[authStore] profiles 查询失败:",t.message,"| code:",i),h=O();return}if(!e){console.log("[authStore] profiles 表无记录，自动创建 (upsert)...");const i=((n=c.user_metadata)==null?void 0:n.nickname)||((r=c.user_metadata)==null?void 0:r.display_name)||((a=c.email)==null?void 0:a.split("@")[0])||"";try{const l=await B(g.from("profiles").upsert({id:c.id,display_name:i,avatar_url:((s=c.user_metadata)==null?void 0:s.avatar_url)||"",bio:""},{onConflict:"id",ignoreDuplicates:!1}),8e3,"profiles 创建超时");if(l.error){console.warn("[authStore] 自动创建 profile 失败:",l.error.message,"| code:",l.error.code),h=O();return}try{const p=await B(g.from("profiles").select("*").eq("id",c.id).maybeSingle(),5e3,"profiles 二次查询超时");if(p.error)console.warn("[authStore] 二次查询 profile 出错:",p.error.message);else if(p.data){h=p.data,console.log("[authStore] profile 自动创建并查询成功");return}}catch(p){console.warn("[authStore] 二次查询 profile 异常:",p.message)}}catch(l){console.warn("[authStore] 自动创建 profile 异常:",l.message)}h=O();return}h=e}function O(){var e,t,o,n,r;return c?{id:c.id,username:((e=c.user_metadata)==null?void 0:e.display_name)||((t=c.email)==null?void 0:t.split("@")[0])||"",display_name:((o=c.user_metadata)==null?void 0:o.display_name)||((n=c.email)==null?void 0:n.split("@")[0])||"",avatar_url:((r=c.user_metadata)==null?void 0:r.avatar_url)||null,bio:""}:null}function $(){const e=we();te.forEach(t=>{try{t(e)}catch(o){console.warn("[authStore] 订阅回调出错:",o)}})}function we(){return{user:c,profile:h,loading:k}}function B(e,t,o){return Promise.race([e,new Promise((n,r)=>setTimeout(()=>r(new Error(o)),t))])}let f=null;function Te(e){f=e}async function Me(e,t){const{count:o,error:n}=await f.from("likes").select("*",{count:"exact",head:!0}).eq("user_id",e).eq("spot_id",t);if(n)throw n;return o>0}async function Ae(e,t){try{const{data:o,error:n}=await f.from("likes").insert({user_id:e,spot_id:t});if(n)throw n;return o}catch(o){throw console.error("[db] likeSpot 失败:",o),new Error(M(o,"点赞失败，请检查权限或重试"))}}async function Pe(e,t){try{const{data:o,error:n}=await f.from("likes").delete().eq("user_id",e).eq("spot_id",t);if(n)throw n;return o}catch(o){throw console.error("[db] unlikeSpot 失败:",o),new Error(M(o,"取消点赞失败，请检查权限或重试"))}}async function $e(e){const{count:t,error:o}=await f.from("likes").select("*",{count:"exact",head:!0}).eq("spot_id",e);if(o)throw o;return t||0}async function Ue(e,t){const{count:o,error:n}=await f.from("favorites").select("*",{count:"exact",head:!0}).eq("user_id",e).eq("spot_id",t);if(n)throw n;return o>0}async function Be(e,t){try{const{data:o,error:n}=await f.from("favorites").insert({user_id:e,spot_id:t});if(n)throw n;return o}catch(o){throw console.error("[db] favoriteSpot 失败:",o),new Error(M(o,"收藏失败，请检查权限或重试"))}}async function Ne(e,t){try{const{data:o,error:n}=await f.from("favorites").delete().eq("user_id",e).eq("spot_id",t);if(n)throw n;return o}catch(o){throw console.error("[db] unfavoriteSpot 失败:",o),new Error(M(o,"取消收藏失败，请检查权限或重试"))}}async function De(e){const{count:t,error:o}=await f.from("favorites").select("*",{count:"exact",head:!0}).eq("spot_id",e);if(o)throw o;return t||0}async function He(e){const{data:t,error:o}=await f.from("comments").select("*").eq("spot_id",e).order("created_at",{ascending:!1});if(o)throw o;return t}async function Fe(e,t,o){try{const{data:n,error:r}=await f.from("comments").insert({user_id:e,spot_id:t,content:o}).select();if(r)throw r;return n}catch(n){throw console.error("[db] addComment 失败:",n),new Error(M(n,"评论发表失败，请检查权限或重试"))}}async function Oe(e,t){try{const{data:o,error:n}=await f.from("comments").delete().eq("id",e).eq("user_id",t);if(n)throw n;return o}catch(o){throw console.error("[db] deleteComment 失败:",o),new Error(M(o,"评论删除失败，请检查权限或重试"))}}async function ze(e){const{count:t,error:o}=await f.from("comments").select("*",{count:"exact",head:!0}).eq("spot_id",e);if(o)throw o;return t||0}async function Re(e){const{count:t,error:o}=await f.from("spots").select("*",{count:"exact",head:!0}).eq("creator_id",e);if(o)throw o;return t||0}async function Ke(e){const{count:t,error:o}=await f.from("likes").select("*",{count:"exact",head:!0}).eq("user_id",e);if(o)throw o;return t||0}async function Je(e){const{data:t,error:o}=await f.from("spots").select("views").eq("creator_id",e);if(o)throw o;return(t||[]).reduce((n,r)=>n+(r.views||0),0)}async function je(e){const{data:t,error:o}=await f.from("spots").select("*").eq("creator_id",e).order("created_at",{ascending:!1});if(o)throw o;return t}async function Ze(e){const{data:t,error:o}=await f.from("favorites").select("*").eq("user_id",e).order("created_at",{ascending:!1});if(o)throw o;return t}async function Ge(e){const{data:t,error:o}=await f.from("spot_images").select("*").eq("spot_id",e).order("created_at",{ascending:!1});if(o)throw o;return t}async function We(e,t,o,n){try{const{data:r,error:a}=await f.from("spot_images").insert({spot_id:e,user_id:t,storage_path:o,url:n}).select();if(a)throw a;return r}catch(r){throw console.error("[db] saveSpotImage 失败:",r),new Error(M(r,"图片保存失败，请检查存储权限或重试"))}}async function Ye(e,t){try{const{data:o,error:n}=await f.from("spot_images").delete().eq("id",e).eq("user_id",t);if(n)throw n;return o}catch(o){throw console.error("[db] deleteSpotImage 失败:",o),new Error(M(o,"图片删除失败，请检查权限或重试"))}}function M(e,t){const o=e==null?void 0:e.code,n=(e==null?void 0:e.message)||"",r={42501:"权限不足，请检查数据库 RLS 策略",23505:"数据已存在，请勿重复操作",23503:"关联数据不存在，请检查后重试","42P01":"数据表不存在，请联系管理员",PGRST301:"认证已过期，请重新登录"};return o&&r[o]?r[o]:n.includes("JWT")?"认证已过期，请重新登录":n.includes("network")||n.includes("fetch")?"网络连接异常，请检查网络":n.includes("timeout")||n.includes("超时")?"请求超时，请检查网络后重试":t}async function Ve(e){await m.init(e),ot(),m.subscribe(t=>{Ee(t)})}function Xe(e){return m.subscribe(({user:t,profile:o})=>{e(t,o)})}function H(){return m.isLoggedIn}async function Qe(e,t){return m.signIn(e,t)}async function et(e,t,o){return m.signUp(e,t,o)}async function Se(){return m.signOut()}async function tt(e){return m.updateProfile(e)}function ot(){const e=document.createElement("div");e.id="auth-user-btn",e.innerHTML=`
  ...
  `,document.body.appendChild(e),e.querySelector(".auth-modal-overlay").addEventListener("click",j),e.querySelector(".auth-modal-close").addEventListener("click",j),e.querySelector("#auth-switch-register").addEventListener("click",()=>W("register")),e.querySelector("#auth-switch-login").addEventListener("click",()=>W("login")),e.querySelector("#auth-login-submit").addEventListener("click",async()=>{const t=e.querySelector("#auth-login-email").value.trim(),o=e.querySelector("#auth-login-password").value,n=e.querySelector("#auth-login-error"),r=e.querySelector("#auth-login-submit");if(!t||!o){n.textContent="请填写邮箱和密码";return}if(!m.supabase){n.textContent="服务未初始化，请刷新页面";return}n.textContent="",r.disabled=!0,r.textContent="登录中...";try{await Qe(t,o),j(),ue()}catch(a){n.textContent=pe(a.message)}finally{r.disabled=!1,r.textContent="登录"}}),e.querySelector("#auth-register-submit").addEventListener("click",async()=>{const t=e.querySelector("#auth-register-displayname").value.trim(),o=e.querySelector("#auth-register-email").value.trim(),n=e.querySelector("#auth-register-password").value,r=e.querySelector("#auth-register-error"),a=e.querySelector("#auth-register-submit");if(!t||!o||!n){r.textContent="请填写所有字段";return}if(n.length<6){r.textContent="密码至少需要6位";return}if(!m.supabase){r.textContent="服务未初始化，请刷新页面";return}r.textContent="",a.disabled=!0,a.textContent="注册中...";try{const{session:s}=await et(o,n,t);if(s)j(),ue();else{r.style.color="rgba(80, 230, 140, 0.95)",r.textContent="注册成功！请查看邮箱确认链接";const i=setTimeout(()=>{r.style.color="",W("login")},2e3);e.dataset._registerTimer=String(i)}}catch(s){r.textContent=pe(s.message)}finally{a.disabled=!1,a.textContent="注册"}}),e.addEventListener("keydown",t=>{t.key==="Enter"&&(e.querySelector("#auth-form-login").style.display!=="none"?e.querySelector("#auth-login-submit").click():e.querySelector("#auth-register-submit").click())})}function nt(){const e=document.createElement("div");e.id="edit-profile-modal",e.className="auth-modal",e.innerHTML=`
    <div class="auth-modal-overlay"></div>
    <div class="auth-modal-panel">
      <button class="auth-modal-close">&times;</button>
      <div class="auth-form">
...
`;document.body.appendChild(d);const ht=d.querySelector(".sidebar-overlay"),yt=d.querySelector(".sidebar-close"),gt=d.querySelector(".hero-placeholder"),bt=d.querySelector(".hero-name"),vt=d.querySelector(".hero-desc"),Y=d.querySelector(".sidebar-photos-area"),oe=d.querySelector("#photo-upload-btn"),U=d.querySelector("#photo-file-input"),C=d.querySelector("#photo-upload-status"),V=d.querySelector(".sidebar-comments-area"),me=d.querySelector("#comment-login-prompt"),A=d.querySelector("#comment-input"),I=d.querySelector("#comment-submit-btn"),S=d.querySelector("#comment-form-status"),z=d.querySelector("#btn-like"),fe=d.querySelector("#btn-like-icon"),he=d.querySelector("#btn-like-text"),wt=d.querySelector("#btn-like-count"),R=d.querySelector("#btn-fav"),ye=d.querySelector("#btn-fav-icon"),ge=d.querySelector("#btn-fav-text"),St=d.querySelector("#btn-fav-count"),xt=d.querySelector("#btn-comment-jump"),Ct=d.querySelector("#btn-comment-count");function ae(){d.classList.remove("open")}ht.addEventListener("click",ae);yt.addEventListener("click",ae);async function Et(e,t){ae(),G();const o=document.createElement("div");o.id="spot-pioneer-guide",o.className="spot-pioneer-guide",o.innerHTML=`
  ...
  `,document.body.appendChild(o);const n=o.querySelector(".spot-pioneer-overlay"),r=o.querySelector(".spot-pioneer-close"),a=o.querySelector("#pioneer-submit"),s=o.querySelector("#pioneer-status"),i=o.querySelector("#pioneer-name"),l=o.querySelector("#pioneer-desc"),p=o.querySelector(".spot-pioneer-coords");n.addEventListener("click",G),r.addEventListener("click",G),p.textContent="📍 正在定位...";try{const w=`https://restapi.amap.com/v3/geocode/regeo?key=7dfc44451d8128e329100a0c71fa90b6&location=${e},${t}&extensions=base`,x=await(await fetch(w)).json();if(x.status==="1"&&x.regeocode){const E=x.regeocode.formatted_address||"",b=x.regeocode.addressComponent,Q=(b==null?void 0:b.township)||(b==null?void 0:b.district)||(b==null?void 0:b.city)||E||"";i.placeholder=`如：${Q||"此处"}`,p.textContent=`📍 ${E||`${e.toFixed(4)}, ${t.toFixed(4)}`}`}else p.textContent=`📍 ${e.toFixed(4)}, ${t.toFixed(4)}`}catch{p.textContent=`📍 ${e.toFixed(4)}, ${t.toFixed(4)}`}a.addEventListener("click",async()=>{if(!H()){P();return}const w=i.value.trim(),L=l.value.trim();if(!w){s.textContent="请输入景区名称",s.style.color="rgba(255, 120, 120, 0.95)";return}if(!L){s.textContent="请写下一句话打卡心得",s.style.color="rgba(255, 120, 120, 0.95)";return}s.textContent="正在创建...",s.style.color="rgba(255, 255, 255, 0.7)",a.disabled=!0,a.textContent="⏳ 创建中...";try{const{data:x,error:E}=await T.from("spots").insert({name:w,longitude:e,latitude:t,description:L,creator_id:y.id}).select();if(E)throw E;const b=x[0];se(b),G(),q.setZoomAndCenter(12,[e,t]),await ie(b.id,b.name,b.description)}catch(x){console.error("[pioneer] 创建景区失败:",x),s.textContent="创建失败："+(x.message||"请检查网络后重试"),s.style.color="rgba(255, 120, 120, 0.95)",a.disabled=!1,a.textContent="✨ 立即点亮并分享"}}),o.addEventListener("keydown",w=>{w.key==="Enter"&&!a.disabled&&a.click()})}function G(){const e=document.getElementById("spot-pioneer-guide");e&&e.remove()}function se(e){const t=new AMap.Marker({position:[e.longitude,e.latitude],title:e.name,label:{content:`<div style="color:#fff;font-size:12px;text-shadow:0 1px 2px rgba(0,0,0,0.8);white-space:nowrap">${K(e.name)}</div>`,direction:"top",offset:new AMap.Pixel(0,-5)},extData:{id:e.id,name:e.name,description:e.description}});t.on("click",()=>Lt(t)),q.add(t)}async function _t(){const{data:e,error:t}=await T.from("spots").select("*");if(t){console.error("加载景区数据失败:",t),alert("加载景区数据失败，请检查 Supabase 配置");return}qt(),e.forEach(se),e.length>0&&q.setZoomAndCenter(10,[e[0].longitude,e[0].latitude])}function qt(){q.clearMap()}async function Lt(e){_e=Date.now();const t=e.getExtData();!t||!t.id||(q.setZoomAndCenter(12,e.getPosition()),await ie(t.id,t.name,t.description))}async function ie(e,t,o){u=Number(e),bt.textContent=t||"",vt.textContent=o||"暂无介绍",gt.style.display="flex",Y.innerHTML="",V.innerHTML="",S.textContent="",d.classList.add("open"),qe(),await kt(),le(u),await X(u)}z.addEventListener("click",async()=>{if(!y){P();return}if(u){z.disabled=!0;try{N?(await Pe(y.id,u),N=!1):(await Ae(y.id,u),N=!0),await J(),ce()}catch(e){console.error("点赞操作失败:",e)}finally{z.disabled=!1}}});R.addEventListener("click",async()=>{if(!y){P();return}if(u){R.disabled=!0;try{D?(await Ne(y.id,u),D=!1):(await Be(y.id,u),D=!0),await J(),ce()}catch(e){console.error("收藏操作失败:",e)}finally{R.disabled=!1}}});xt.addEventListener("click",()=>{const e=document.getElementById("comment-form-wrapper");e&&(e.scrollIntoView({behavior:"smooth",block:"center"}),H()?setTimeout(()=>A.focus(),400):P())});function ce(){N?(fe.textContent="❤️",he.textContent="已赞",z.classList.add("active")):(fe.textContent="🤍",he.textContent="点赞",z.classList.remove("active")),D?(ye.textContent="⭐",ge.textContent="已收藏",R.classList.add("active")):(ye.textContent="☆",ge.textContent="收藏",R.classList.remove("active"))}async function J(){if(u)try{const[e,t,o]=await Promise.all([$e(u),De(u),ze(u)]);wt.textContent=e>0?e:"",St.textContent=t>0?t:"",Ct.textContent=o>0?o:""}catch(e){console.warn("刷新计数失败:",e)}}async function kt(){if(!y||!u)N=!1,D=!1;else try{const[e,t]=await Promise.all([Me(y.id,u),Ue(y.id,u)]);N=e,D=t}catch(e){console.warn("刷新互动状态失败:",e)}ce(),await J()}function P(){const e=document.getElementById("auth-modal");e&&e.classList.add("open")}function be(){const e=document.getElementById("add-form-login-prompt"),t=document.getElementById("field-address"),o=document.getElementById("field-desc"),n=document.getElementById("add-submit");!e||!t||!o||!n||(H()?(e.style.display="none",t.disabled=!1,o.disabled=!1,n.disabled=!1,n.textContent="分享我的足迹",t.placeholder="景区名称或详细地址（如：杭州西湖）",o.placeholder="景区游记或一句话介绍"):(e.style.display="block",t.disabled=!0,o.disabled=!0,n.disabled=!0,n.textContent="请先登录",t.placeholder="请登录后再分享",o.placeholder="请登录后再分享"))}function qe(){H()?(me.style.display="none",A.disabled=!1,I.disabled=!1,I.textContent="发表评论",A.placeholder="写下你的评论..."):(me.style.display="block",A.disabled=!0,I.disabled=!0,I.textContent="请先登录",A.placeholder="请先登录后再发表评论")}async function le(e){const[t,o]=await Promise.allSettled([T.from("user_stories").select("photo_urls").eq("spot_id",e).order("created_at",{ascending:!1}),Ge(e)]),n=[];if(t.status==="fulfilled"&&t.value.data&&t.value.data.forEach(r=>{r.photo_urls&&Array.isArray(r.photo_urls)&&r.photo_urls.forEach(a=>n.push({url:a,source:"story"}))}),o.status==="fulfilled"&&o.value&&o.value.forEach(r=>{n.push({url:r.url,source:"upload",id:r.id,userId:r.user_id})}),Y.innerHTML="",n.length>0){const r=document.createElement("div");r.className="photo-grid",[...new Map(n.map(s=>[s.url,s])).values()].forEach(s=>{const i=document.createElement("div");if(i.className="photo-item",i.innerHTML=`<img src="${K(s.url)}" alt="景区照片" loading="lazy" />`,s.source==="upload"&&y&&s.userId===y.id){const l=document.createElement("button");l.className="photo-delete-btn",l.textContent="×",l.title="删除此照片",l.addEventListener("click",async p=>{if(p.stopPropagation(),!!confirm("确定要删除这张照片吗？"))try{await Ye(s.id),le(e)}catch(w){console.error("删除照片失败:",w)}}),i.appendChild(l)}r.appendChild(i)}),Y.appendChild(r)}else Y.innerHTML='<div class="photo-empty">快来上传第一张照片吧！</div>'}oe.addEventListener("click",()=>{if(!y){P();return}u&&U.click()});U.addEventListener("change",async()=>{const e=U.files[0];if(!e)return;if(!["image/jpeg","image/png","image/webp"].includes(e.type)){C.textContent="仅支持 JPG / PNG / WEBP 格式",C.style.color="rgba(255, 80, 80, 0.95)",U.value="";return}if(e.size>5*1024*1024){C.textContent="图片不能超过 5MB",C.style.color="rgba(255, 80, 80, 0.95)",U.value="";return}C.textContent="正在上传...",C.style.color="rgba(255, 255, 255, 0.7)",oe.disabled=!0;try{const o=e.name.split(".").pop().toLowerCase(),n=Date.now(),r=`${y.id}/${u}/${n}.${o}`,{error:a}=await T.storage.from("spot-images").upload(r,e,{upsert:!1});if(a)throw a;const{data:s}=T.storage.from("spot-images").getPublicUrl(r),i=s.publicUrl;await We(u,y.id,r,i),C.textContent="上传成功！",C.style.color="rgba(80, 230, 140, 0.95)",await le(u),setTimeout(()=>{C.textContent=""},2e3)}catch(o){console.error("上传照片失败:",o),C.textContent="上传失败："+(o.message||"未知错误"),C.style.color="rgba(255, 80, 80, 0.95)"}finally{oe.disabled=!1,U.value=""}});async function X(e){let t;try{t=await He(e)}catch(r){console.warn("加载评论失败:",r),t=[]}const o=document.getElementById("comments-title");if(o&&(o.textContent=`评论 (${t.length})`),V.innerHTML="",t.length===0){V.innerHTML='<div class="comment-empty">暂无评论，来说两句吧</div>';return}const n=document.createElement("div");n.className="comment-list",t.forEach(r=>{const a=It(r.created_at),s=r.avatar_url||`https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(r.user_id)}`,i=y&&r.user_id===y.id,l=document.createElement("div");l.className="comment-bubble",l.innerHTML=`
      <div class="comment-header">
        <img class="comment-avatar" src="${K(s)}" alt="" />
        <span class="comment-author-name">${K(r.display_name)}</span>
        <span class="comment-time">${a}</span>
        ${i?`<button class="comment-delete-btn" data-id="${r.id}">删除</button>`:""}
      </div>
      <div class="comment-text">${K(r.content)}</div>
    `,n.appendChild(l)}),V.appendChild(n),n.querySelectorAll(".comment-delete-btn").forEach(r=>{r.addEventListener("click",async()=>{if(!confirm("确定要删除这条评论吗？"))return;const a=Number(r.dataset.id);r.disabled=!0;try{await Oe(a),await X(e),await J()}catch(s){console.error("删除评论失败:",s),r.disabled=!1}})})}function It(e){if(!e)return"";const t=new Date(e),n=new Date-t,r=Math.floor(n/6e4);if(r<1)return"刚刚";if(r<60)return`${r}分钟前`;const a=Math.floor(r/60);if(a<24)return`${a}小时前`;const s=Math.floor(a/24);return s<30?`${s}天前`:t.toLocaleDateString("zh-CN")}function K(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}I.addEventListener("click",async()=>{if(!u){S.textContent="请先点击地球上的景区",S.style.color="rgba(255, 80, 80, 0.95)";return}if(!H()){P();return}const e=A.value.trim();if(!e){S.textContent="请输入评论内容",S.style.color="rgba(255, 80, 80, 0.95)";return}S.textContent="正在发表...",S.style.color="rgba(255, 255, 255, 0.8)",I.disabled=!0;try{await Fe(y.id,u,e)}catch(t){console.error("发表评论失败:",t),S.textContent="发表失败："+t.message,S.style.color="rgba(255, 80, 80, 0.95)",I.disabled=!1;return}S.textContent="发表成功！",S.style.color="rgba(80, 230, 140, 0.95)",A.value="",await X(u),await J(),I.disabled=!1,setTimeout(()=>{S.textContent=""},2e3)});const _=document.createElement("div");_.id="add-form";_.innerHTML=`
